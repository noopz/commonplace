...
// ---- Step 4.5: Delegate to discoverGenres lib (also exposed as
// `commonplace discover-genres` and re-run incrementally by the indexer). ----

// Concept/MOC dirs have their own classification and aren't lintable as
// sources — exclude from path-prefix discovery. Sources dir IS a meaningful
// genre (research papers, articles, etc.) and stays in.
const genreStructureDirs = new Set(
  [merged.structure.concepts, merged.structure.mocs].filter(Boolean),
);

// Reuse init's existing 200-file frontmatter sample; the lib only needs
// {relDir, fm} per note.
const genreResult = discoverGenres(
  samples.map((s) => ({ relDir: s.relDir, fm: s.fm })),
  genreStructureDirs,
  config.wikiPath,
);
...
const untunedGenres = genreResult.untunedGenres;
const discoveredGenres = genreResult.conventions.genres;
