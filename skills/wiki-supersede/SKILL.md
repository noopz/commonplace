---
name: wiki-supersede
description: "Use this skill any time the user retires, replaces, deprecates, or decommissions an entity that has prose mentions across the vault. Without it, siblings keep treating the retired entity as live — a known recurring failure of the wiki-ingest workflow alone. Activate on utterances like \"mark X as retired/deprecated/replaced by Y\", \"we switched from X to Y\", \"X is gone now, use Y instead\", \"decommissioned X\". Also activate when ingest of a new note declares supersession (`supersedes`, `replaces`, `replaced`, `migrated from`, `formerly`, `previously known as`, `in place of`) of an existing vault entity. Also activate when a note's frontmatter gains a `retired` tag or a `> [!warning] Retired` callout, or when wiki-query/wiki-pruner/wiki-impact-checker reports a retired-but-still-referenced entity. Do NOT activate for stale-link cleanup (broken wikilinks without a successor), one-off typo renames, or live entities still in use."
---
