...
1. **Ask for name, scope, and linkGroup**. Name should be kebab-case (e.g., `home-automation`, `poe2-builds`). Scope is `public` or `private`. If unsure, default to `public` for technical topics, `private` for personal/hobby topics. If the domain is related to other private domains, ask which linkGroup it belongs to.
